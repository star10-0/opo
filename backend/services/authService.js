...
export const loginUser = async (payload = {}) => {
  ...
  let user = await User.findOne({ email }).select("_id name email password role permissions stationAccess allowedStations accountType station organization currentStation isActive");

  if (!user) {
    await ensureInitialAdminIfEmpty();
    user = await User.findOne({ email }).select("_id name email password role permissions stationAccess allowedStations accountType station organization currentStation isActive");
  }
  ...
  if (accountType && user.accountType !== accountType) {
    const expected = user.accountType === "company" ? "مؤسسة متعددة المحطات" : "محطة فردية";
    throw buildError(`نوع الحساب المحدد لا يطابق حسابك. النوع الصحيح: ${expected}`, 401);
  }
